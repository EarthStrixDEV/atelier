...
Tailwind v4 with the theme defined in `src/index.css` via `@theme`. The palette is **18 tokens** — the 11 legacy names (`--color-bg`, `--color-surface`, `--color-surface-2`, `--color-border`, `--color-border-strong`, `--color-text`, `--color-text-dim`, `--color-text-faint`, `--color-accent`, `--color-accent-ink`, `--color-danger`) plus `--color-success`, `--color-warning`, `--color-info`, `--color-overlay`, `--color-media-scrim`, `--color-on-media`, `--color-on-media-dim`. Always use the mapped utilities (`bg-surface`, `text-text-dim`, `border-border-strong`, `bg-overlay`, `text-on-media`, …) — **never hardcode a color**, or it won't follow the theme.

**Themes** — 7 picker options: 6 palettes (`paper` light default, `ink`, `contrast`, `sepia`, `midnight`, `nocturne`) plus `system`, which resolves to `paper`/`ink` via `prefers-color-scheme`. `src/lib/themes.ts` is the **source of truth for color values** (`THEMES`, `DEFAULT_THEME`, `CSS_VAR_BY_TOKEN`); `src/index.css` mirrors them as `[data-theme="…"]` blocks that override the `@theme` defaults — change a color and you must change **both**. Switching a theme just sets `document.documentElement.dataset.theme`; the choice persists in `localStorage` under `atelier_theme`, and an inline script in `index.html` applies it before first paint (FOUC guard) — it can't `import`, so `"paper"` is duplicated there on purpose. `--logo-invert` (outside `@theme`, since it's not a color) is `1` on light themes and `0` on dark ones, for the logo's `filter: invert()`.
